(function ($) {
    ...
    function ReferrersReportIndex() {
        ...
        function WizardValidate() {
            ...
            $(".next a").on("click", function () {
                ...
                if ($("#ReferrersId").val() == "") {
                    if ($("#ReferrersId").val() == "") {
                        $('span[data-valmsg-for="ReferrersId"]').text('*required');
                    }
                ...
                if ($("#rootwizard li.active").find('a').attr('href') == "#tab1") {
                    ...
                    if (IsStepOneValid) {
                        var option = {};
                        option.ReferrersId= $('#ReferrersId').val();
                        option.IncludeFamailyMemberDetails = $('#IncludeFamailyMemberDetails').is(":checked");;
                        $(".img-loading-div").show();
                        $.post(Global.DomainName + "ReferrerReport/GetMembersByStepOneWizardValues", option, function (data) {
                            var html = "";
                            $("#grid-family-list-step-two tbody").html('');
                            if (!data.isSuccess) {
        ...
        function GenerateReport() {
            ...
            if (familyIds.length > 0) {
                ...
                if (frmRoleListOfPeople.valid()) {
                    $.ajax({
                        url: Global.DomainName + 'ReferrerReport/Index',
    ...
    $(function () {
        var self = new ReferrersReportIndex();
        self.init();
    });
}(jQuery));